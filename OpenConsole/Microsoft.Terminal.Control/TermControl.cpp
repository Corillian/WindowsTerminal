...
static_assert(sizeof(winrt::Microsoft::Terminal::Core::ControlKeyStates) == sizeof(::Microsoft::Terminal::Core::ControlKeyStates), "Mismatch between ControlKeyStates sizes.");
...
namespace winrt::Microsoft::Terminal::Control::implementation
{
    static bool tryShowInputPane()
    {
        ...
        if(window)
        {
            ...
            if(SUCCEEDED(window.as<IWindowNative>()->get_WindowHandle(&hWnd)))
            {
                winrt::Windows::UI::ViewManagement::InputPane inputPane = wil::capture_interop<winrt::Windows::UI::ViewManagement::InputPane>(&IInputPaneInterop::GetForWindow, hWnd);
    ...
    {
        // NOTE: For WinUI3 the documentation says that a SwapChainPanel must first
        // be cast to IInspectable or IUnknown before you can retrieve the ISwapChainPanelNative
        // interface
        SwapChainPanel().as<IInspectable>().as<ISwapChainPanelNative2>()->SetSwapChainHandle(swapChainHandle);
    }
                                        ...
                                        const Input::CharacterReceivedRoutedEventArgs& e)
    {
        ...
        const auto handled = _core.SendCharEvent(ch, scanCode, *reinterpret_cast<winrt::Microsoft::Terminal::Core::ControlKeyStates*>(&modifiers));
        e.Handled(handled);
    }
        ...
        const bool keyDown)
    {
        ...
        const auto handled = vkey ?
            _core.TrySendKeyEvent(vkey,
                scanCode,
                *reinterpret_cast<winrt::Microsoft::Terminal::Core::ControlKeyStates const*>(&modifiers),
                keyDown) :
            true;
        ...
        const bool rightButtonDown)
    {
        ...
        return _interactivity.MouseWheel(*reinterpret_cast<winrt::Microsoft::Terminal::Core::ControlKeyStates const*>(&modifiers), delta, _toTerminalOrigin(location).to_core_point(), state);
    }
    ...
    bool TermControl::_CapturePointer(const Windows::Foundation::IInspectable& sender, const Microsoft::UI::Xaml::Input::PointerRoutedEventArgs& args)
    {
        IUIElement uielem;
    ...
    bool TermControl::_ReleasePointerCapture(const Windows::Foundation::IInspectable& sender, const Microsoft::UI::Xaml::Input::PointerRoutedEventArgs& args)
    {
        IUIElement uielem;
        if(sender.try_as(uielem))
        {
    ...
    void TermControl::_TryStartAutoScroll(const Microsoft::UI::Input::PointerPoint& pointerPoint, const double scrollVelocity)
    {
    ...
    void TermControl::_SwapChainScaleChanged(const Microsoft::UI::Xaml::Controls::SwapChainPanel& sender,
    ...
    ControlKeyStates TermControl::_GetPressedModifierKeys() noexcept
    {
        ...
        for(const auto& mod : modifiers)
        {
            ...
            const auto isDown = WI_IsFlagSet(static_cast<uint32_t>(state), static_cast<uint32_t>(winrt::Windows::UI::Core::CoreVirtualKeyStates::Down));
        ...
        {
            const auto state = winrt::Microsoft::UI::Input::InputKeyboardSource::GetKeyStateForCurrentThread(mod.vkey);
            const auto isLocked = WI_IsFlagSet(static_cast<uint32_t>(state), static_cast<uint32_t>(winrt::Windows::UI::Core::CoreVirtualKeyStates::Locked));
    ...
    void TermControl::_PointerExitedHandler(const Windows::Foundation::IInspectable& /*sender*/,
        const Microsoft::UI::Xaml::Input::PointerRoutedEventArgs& /*e*/)
    {
