...
namespace winrt::Microsoft::Terminal::Control::implementation
{
    struct KeyChord : KeyChordT<KeyChord>
    {
        ...
        KeyChord(const VkModifiers modifiers, int32_t vkey, int32_t scanCode) noexcept;
        KeyChord(bool ctrl, bool alt, bool shift, bool win, int32_t vkey, int32_t scanCode) noexcept;

        uint64_t Hash() const noexcept;
        bool DeepEquals(const Control::KeyChord& other) const noexcept;

        VkModifiers Modifiers() const noexcept;
        void Modifiers(const VkModifiers value) noexcept;
